...
app.get('/health', (_request, response) => response.json({ status: 'UP', service: 'dashboard', instanceId }));
app.get('/instance', (_request, response) => response.json({ instanceId }));

const httpServer = app.listen(port, () => console.log(`${instanceId} ouvindo na porta ${port}`));
const webSocketServer = new WebSocketServer({ server: httpServer, path: '/alerts' });

webSocketServer.on('connection', (socket) => {
  socket.send(JSON.stringify({ type: 'CONNECTED', instanceId, connectedAt: new Date().toISOString() }));
});
...
await channel.consume(queue.queue, (message) => {
  ...
  console.log(`${event.eventId} entregue a ${delivered} painel(is) por ${instanceId}`);
  channel.ack(message);
});
