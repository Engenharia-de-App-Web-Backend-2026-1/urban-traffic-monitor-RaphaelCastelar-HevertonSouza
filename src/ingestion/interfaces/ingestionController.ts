...
export function createIngestionController(analyzerClient: any): Express {
  const app = express();
  app.use(express.json({ limit: "64kb" }));
  app.get("/health", (_request, response) =>
    response.json({ status: "UP", service: "ingestion-api" }),
  );
...
function validateReading(body: Record<string, unknown>): string | null {
  ...
  const numericFields = [
    "latitude", "longitude", "averageSpeedKmh", "vehicleCount", "occupancyPercent",
  ];
